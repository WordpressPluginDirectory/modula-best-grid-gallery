...
final class Modern_Gallery {
	...
	/**
	 * Absolute path to the modern visitor loader script.
	 */
	public static function visitor_renderer_script_path(): string {
		return MODULA_PATH . 'assets/js/front/modula-gallery.js';
	}

	/**
	 * Absolute path to the modern visitor loader stylesheet.
	 */
	public static function visitor_renderer_style_path(): string {
		return MODULA_PATH . 'assets/css/front/modula-gallery.css';
	}

	/**
	 * Whether built modern visitor renderer assets are on disk.
	 */
	public static function visitor_renderer_assets_available(): bool {
		return file_exists( self::visitor_renderer_script_path() )
			&& file_exists( self::visitor_renderer_style_path() );
	}

	/**
	 * Cache-busting version for visitor assets (plugin version + file mtime).
	 */
	public static function visitor_renderer_asset_version(): string {
		$version = defined( 'MODULA_LITE_VERSION' ) ? (string) MODULA_LITE_VERSION : '0';
		$script  = self::visitor_renderer_script_path();
	...
	/**
	 * Visible failure when modern markup cannot run the visitor renderer.
	 */
	public static function missing_visitor_renderer_html(): string {
		$message = esc_html__(
			'This gallery could not load its display scripts. Clear any page/CDN cache, then reload. If it continues, reinstall Modula or contact support.',
			'modula-best-grid-gallery'
		);

		return '<div class="modula modula-gallery-bootstrap-error" role="alert">' . $message . '</div>';
	}

	/**
	 * Enqueue modern visitor loader + chrome (styles/scripts required to leave Loading).
	 *
	 * @param array<string, mixed> $settings          Flat gallery settings (for modula_extra_scripts).
	 * @param array<int, mixed>    $images            Gallery rows (for modula_extra_scripts).
	 * @param bool                 $with_extra_scripts Whether to fire modula_extra_scripts.
	 */
	public static function enqueue_visitor_renderer_assets( array $settings = array(), array $images = array(), bool $with_extra_scripts = true ): void {
		if ( ! self::visitor_renderer_assets_available() ) {
			return;
		}

		$version    = self::visitor_renderer_asset_version();
		$css_handle = 'modula-gallery';
		$css_path   = MODULA_URL . 'assets/css/front/modula-gallery.css';

		if ( ! wp_style_is( $css_handle, 'enqueued' ) ) {
			wp_enqueue_style( $css_handle, $css_path, array(), $version );
		}
		self::attach_gallery_chrome_inline_style( $css_handle );

		$script_handle = 'modula-gallery';
		$script_path   = MODULA_URL . 'assets/js/front/modula-gallery.js';

		if ( ! wp_script_is( $script_handle, 'enqueued' ) ) {
			wp_enqueue_script( $script_handle, $script_path, array(), $version, true );
		}
		...
		wp_localize_script(
			...
			array(
				...
				'strings'      => array(
					...
					'tryAgain'       => esc_html__( 'Try again', 'modula-best-grid-gallery' ),
				),
			)
		);

		self::enqueue_bootstrap_stylesheet();
		self::attach_gallery_chrome_inline_style( 'modula-gallery-bootstrap' );
		self::preload_bootstrap_stylesheet();

		if ( ! $with_extra_scripts ) {
			return;
		}

		/**
		 * Legacy shortcode fires this after enqueue; v2 must too so Pro extensions load.
		 *
